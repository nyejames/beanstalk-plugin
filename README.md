...
- `$html` -> `meta.embedded.block.html.bst`
...
Mixed `$markdown`, `$css`, `$html`, and `$code(...)` directives use a TextMate lookahead heuristic to mimic last-directive-wins for common patterns.
...
`$html` templates treat the template body as HTML with balanced square-bracket parsing while still recognizing nested templates and slot markers (including inside quoted attributes) before HTML fallback.
